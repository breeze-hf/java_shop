    ...
    public String login(String loginName, String passwordMD5, HttpSession httpSession) {
        ...
        if (user != null && httpSession != null) {
            if (user.getLockedFlag() == 1) {
                throw new NewBeeMallException("用户被锁定，不能登录");
            }
            try { //昵称太长 影响页面展示
                ...
                String token = Base64Utils.encodeToString(user.getUserId().toString().getBytes("utf-8"));
                return token;
            } catch (Exception e) {
                throw new NewBeeMallException("登录失败");
            }
        } else {
