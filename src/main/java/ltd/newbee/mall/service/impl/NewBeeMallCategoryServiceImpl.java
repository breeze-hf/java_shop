...
public class NewBeeMallCategoryServiceImpl implements NewBeeMallCategoryService {
    ...
    @Override
    public Boolean checkCategories(Integer[] ids) {
        boolean flag = false;
        if (ids.length < 1) {
            return flag;
        }
        flag = goodsCategoryMapper.checkCategories(ids) > 0;
        List<Long> parentIdList = new ArrayList<>();
        for (Integer id : ids) {
            parentIdList.add(Long.valueOf(id));
        }
        List<GoodsCategory> goodsCategories = goodsCategoryMapper.selectParentIds(parentIdList);
        flag = goodsCategories.size() > 0;
        return flag;
    }
